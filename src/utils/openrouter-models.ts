...
export function formatModelCostInfo(model: OpenRouterModel): string {
	...
	const promptCost = parseFloat(model.pricing.prompt) * 1000 * 1000; // Convert from per 1K to per 1M
	const completionCost = parseFloat(model.pricing.completion) * 1000 * 1000; // Convert from per 1K to per 1M
