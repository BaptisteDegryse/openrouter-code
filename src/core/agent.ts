	...
	async chat(userInput: string): Promise<void> {
		...
		if (!this.client) {
			...
			if (envApiKey) {
				...
				// Try config file
				debugLog(
					'Environment variable OPENROUTER_API_KEY not found, checking config file',
				);
				...
				if (configApiKey) {
					...
					throw new Error(
						'No API key available. Please use /login to set your OpenRouter API key.',
					);
				}
			}
		...
		while (true) {
			...
			while (iteration < maxIterations) {
				...
				try {
					...
					iterationSummary.finishReason = response.choices[0]
					...
					if (message.tool_calls) {
						...
						if (message.content || reasoning) {
							...
							iterationSummary.thinkingPreview = previewSource.substring(
								0,
								160,
							);
						}
						...
						const toolExecutionPromises = message.tool_calls.map(
							async toolCall => {
